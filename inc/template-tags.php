 ...
 */
function plume_site_logo($args = array(), $echo = true) {
	...
	$defaults = array(
		...
		'title'       => '<a href="%1$s" data-title="%2$s">%2$s</a>',
	...
	/**
	 * Filters the arguments for `plume_site_logo()`.
	 *
	 * @param array  $args     Parsed arguments.
	 * @param array  $defaults Function's default arguments.
	 */
	$args = apply_filters('plume_site_logo_args', $args, $defaults);
	 ...
	 */
	$html = apply_filters('plume_site_logo', $html, $args, $classname, $contents);
 ...
 */
function plume_site_description($echo = true) {
	 ...
	 */
	$html = apply_filters('plume_site_description', $html, $description, $wrapper);
...
function plume_get_post_meta($post_id = null, $location = 'single-top') {
	 ...
	 */
	$disallowed_post_types = apply_filters('plume_disallowed_post_types_for_meta_output', array('page'));
	...
	if ('single-top' === $location) {
		...
		$post_meta = apply_filters(
			'plume_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		 ...
		 */
		$post_meta = apply_filters(
			'plume_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
	...
	if ($post_meta && ! in_array('empty', $post_meta, true)) {
		...
		<div class="post-meta-wrapper<?php echo esc_attr($post_meta_wrapper_classes); ?>">
			...
			<ul class="post-meta<?php echo esc_attr($post_meta_classes); ?>">
				 ...
				 *
				 * @since Plume 1.0
				 * @since Plume 1.1 Added the `$post_meta` and `$location` parameters.
				 *
				 * @param int    $post_id   Post ID.
				 ...
				 */
				do_action('plume_start_of_post_meta_list', $post_id, $post_meta, $location);
				...
				if (in_array('post-date', $post_meta, true)) {
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							...
							<?php plume_the_theme_svg('calendar'); ?>
						</span>
						<span class="meta-text">
				...
				if (in_array('categories', $post_meta, true) && has_category()) {
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							...
							<?php plume_the_theme_svg('folder'); ?>
						</span>
						<span class="meta-text">
							<?php _ex('In', 'A string that is output before one or more categories', 'plume'); ?> <?php the_category(', '); ?>
						</span>
					</li>
					<?php
 ...
 */
function plume_filter_wp_list_pages_item_classes($css_class, $page, $depth, $args, $current_page) {
 ...
 */
function plume_add_sub_toggles_to_main_menu($args, $item, $depth) {
 ...
 */
function plume_nav_menu_social_icons($item_output, $item, $depth, $args) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ('social' === $args->theme_location) {
		$svg = Plume_SVG_Icons::get_social_link_svg($item->url);
		if (empty($svg)) {
			$svg = plume_get_theme_svg('link');
		}
...
add_filter('walker_nav_menu_start_el', 'plume_nav_menu_social_icons', 10, 4);
 ...
 */
function plume_body_classes($classes) {
...
add_filter('body_class', 'plume_body_classes');
 ...
 */
function plume_get_the_archive_title($title) {

	$regex = apply_filters(
		'plume_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter('get_the_archive_title', 'plume_get_the_archive_title');
 ...
 */
function plume_toggle_duration() {
	/**
	 ...
	 */
	$duration = apply_filters('plume_toggle_duration', 250);
 ...
 */
function plume_unique_id($prefix = '') {
	static $id_counter = 0;
